...
export const updateWebhook = async (id: number, data: UpdateWebhookRequestDTO) => {
  try {
    const response = await axios.put(`/api/webhooks/${id}`, data, {
      headers: {
...
export const getWebhook = async (id: number): Promise<Webhook> => {
  try {
    const response = await axios.get(`/api/webhooks/${id}`, {
      headers: {
        'Accept': '*/*',
      },
    });
...
export const getWebhooks = async (): Promise<Webhook[]> => {
  try {
    ...
    const response = await axios.get(`/api/webhooks?sectorId=${sectorId}`, {
      headers: {
        'Accept': '*/*',
      },
    });
...
export const deleteWebhook = async (id: number) => {
  try {
    const response = await axios.delete(`/api/webhooks/${id}`);
    return response.data;
  } catch (error) {
