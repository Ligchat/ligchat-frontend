...
export const validateToken = async (token: string) => {
  try {
    const response = await axios.get(`/server/api/auth/validate`, { params: { token } });
...
export const sendVerificationCode = async (data: SendVerificationCodeRequestDTO) => {
  try {
    const response = await axios.post('/server/api/auth/send-verification-code', data);
    return response.data;
  } catch (error) {
...
export const verifyCode = async (data: VerifyCodeRequestDTO) => {
  try {
    const response = await axios.post('/server/api/auth/verify-code', data);
